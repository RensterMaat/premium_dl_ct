...
from src.data import DataModule, CENTERS
...
class TrainedModel(pl.LightningModule):
    def __init__(self, run_id, predictions_save_file_path, fold):
        super().__init__()
        self.model = nets.SEResNet50(spatial_dims=3, in_channels=1, num_classes=1)
    ...
    def test_epoch_end(self, *args, **kwargs):
        if self.predictions_save_file_path.exists():
            other_folds = pd.read_csv(self.predictions_save_file_path).set_index(
                "Unnamed: 0"
            )
            combined_dataframe = other_folds.join(
                self.predictions.to_frame(), how="outer"
            )
    ...
    def get_checkpoint_file(self, run_id):
        checkpoint_root = Path("/mnt/c/Users/user/data/models/benefit")
...
if __name__ == "__main__":
    ...
    for test_center in [c for c in CENTERS if not c in ['amphia','isala','lumc']]:
        ...
        for fold, run_id in fold_vs_id[test_center]:
            ...
            dm = DataModule(
                radiomics_folder, lesion_level_labels_csv, mock_wandb_config
            )
            dm.setup()

            # inference on validation fold for later recalibration and combination model
            model = TrainedModel(
                run_id, save_folder / test_center / "dl_training_preds.csv", fold
            )
            ...
            dm.setup(stage="test")
            model.predictions_save_file_path = (
                save_folder / test_center / "dl_test_preds.csv"
            )
