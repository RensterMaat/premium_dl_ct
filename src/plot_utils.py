...
def plot_calibration_curve(predictor, target, data, ax):
    ...
    ax.bar(pos_bins[:-1] + (neg_bins[1] - neg_bins[0]) / 2, -neg_heights/max(neg_heights.max(), pos_heights.max())/10, width=0.01, color='C1')
    ax.bar(pos_bins[:-1] + (pos_bins[1] - pos_bins[0]) / 2, pos_heights/max(neg_heights.max(), pos_heights.max())/10, width=0.01, color='C0')
